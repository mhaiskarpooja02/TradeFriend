...
class TradeFriendDecisionRunner:
    """
    Phase-1C: Decision Runner
    - Converts PLANNED → READY
    - Produces MorningConfirmReport
    """
    ...
    def __init__(self, ltp_provider=None):
        ...
        self.report = MorningConfirmReport(
            ...
            capital=s["available_swing_capital"] or 0
        )
    ...
    def run(self):
        ...
        for plan in plans:
            ...
            try:
                ...
                result = engine.evaluate(dict(plan))
                ...
                if result["decision"] == "APPROVED":
                    ...
                    self.report.add(
                        symbol=symbol,
                        ltp=result["ltp"] if "ltp" in result else None,
                        entry=trade["entry"],
                        sl=trade["sl"],
                        target=trade["target"],
                        ...
                        confidence=trade.get("confidence") if isinstance(trade, dict) else None
                    )
                    ...
                    self.report.add(
                        ...
                        reason=result["reason"] if "reason" in result else "Rejected"
                    )
